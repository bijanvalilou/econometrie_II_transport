...
table <- Transport_France2019
colnames(table) <- c("Année", "Quantité transportée par routes", "Quantité transportée par train","Prix du disel","Quantité de diesel consommé","PIB","Indice des prix à la consommation (base=2015)","Quantité de diesel consommé des camions","PIB en volume (en milliards d’euros 2014)")
...
sctest(y ~ x, type = "Chow", point = i) %>% pander
}
...
ggplot()+
  ...
  geom_line(aes(x=Transport_France2019$Year, y=y, color="Valeur estimée de y par le modèle"))+
...
ggplot()+
  ...
  geom_line(aes(x=Transport_France2019$Year[1:33], y=y, color="Valeur estimée de y par le modèle"))+
...
white_lm(OLS, interactions = FALSE, statonly = FALSE)
bptest(y ~ x, varformula = NULL, studentize = TRUE, data = list())
...
tstud=qt(0.975,n-K)
#
# Borne inf et sup à 95%
# Lower bound and Upper bound at 95%
#
prevymin=nvy-tstud*sprev
prevymax=nvy+tstud*sprev
cat("prevision - forecast",i, ":",nvy, prevymin, prevymax,sprev, "\n")
rmse=rmse+(yfobs-nvy)*(yfobs-nvy)
}
rmse=sqrt(rmse/nprev)
print(rmse)
```
